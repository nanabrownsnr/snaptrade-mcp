...
def register_extended_tools(mcp, owner_id):
    ...
    def get_orders(account_id: str, status: str = "all") -> dict:
        ...
        return {
            ...
            "orders": _body(c.account_information.get_user_account_orders(**args)),
        }
    ...
    def get_activities(account_id: str) -> dict:
        ...
        return {
            "account_id": account_id,
            "activities": _body(
                c.account_information.get_account_activities(account_id=account_id)
            ),
        }
            ...
            out.append(
                {
                    "account": a,
                    "balances": _body(
                        c.account_information.get_user_account_balance(account_id=aid)
                    ),
                    "positions": _body(
                        c.account_information.get_all_account_positions(account_id=aid)
                    ),
                }
            )
    ...
    def search_symbols(query: str) -> dict:
        ...
        return {
            "query": query,
            "results": _body(
                c.reference_data.symbol_search_user_account(body={"substring": query})
            ),
        }
